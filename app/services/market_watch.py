...
from app.core.config import DATA_DIR, DEFAULT_DATA_PATH, REAL_DATA_PATH
...
def resample_ohlc_frame(frame: pd.DataFrame, rule: str) -> pd.DataFrame:
    ...
    index_column = "date"
    resampled = (
        working.set_index(index_column)
        .resample(rule)
        .agg(aggregations)
    ...
    if not resampled.empty:
        for column in ("symbol", "name", "pool"):
            if column in working.columns:
                resampled[column] = working[column].dropna().iloc[-1] if working[column].dropna().shape[0] else None

    return resampled


def fetch_stock_daily_history_frame(symbol: str, limit: int) -> pd.DataFrame:
    normalized = normalize_symbol(symbol)
    prices = get_local_price_frame(prefer_real=True)
    frame = prices.loc[prices["symbol"].astype(str).str.zfill(6) == normalized].copy()
    if frame.empty:
        return fetch_remote_stock_daily_history_frame(normalized, limit=limit)
...
def fetch_index_daily_history_frame(symbol: str, limit: int) -> pd.DataFrame:
...
def normalize_akshare_minute_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    working = ensure_ohlc_columns(working)
    return working
...
def build_history_payload(symbol: str, period: str = "day", limit: int | None = None) -> dict:
    ...
    if config["mode"] == "daily":
        frame = fetch_index_daily_history_frame(symbol, max(resolved_limit * 3, resolved_limit)) if is_index_symbol(symbol) else fetch_stock_daily_history_frame(symbol, max(resolved_limit * 3, resolved_limit))
        resample_rule = config.get("resample_rule")
        if resample_rule:
            frame = resample_ohlc_frame(frame, str(resample_rule))
...
def search_symbols(query: str, limit: int = 12) -> list[dict]:
    ...
    for item in index_aliases:
        ...
        if keyword in alias_text.lower():
            records.insert(0, {key: value for key, value in item.items() if key != "aliases"})
...
def ensure_watchlist_storage() -> None:
    ...
    if not CUSTOM_WATCHLIST_PATH.exists():
        ...
        CUSTOM_WATCHLIST_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def normalize_watchlist_item(
    ...
    return {
        ...
        "added_at": (added_at or "").strip() or datetime.now().isoformat(timespec="seconds"),
    }
...
def get_custom_watchlist_snapshot() -> dict:
    ...
    items = payload.get("items", [])
    if not items:
        return {"updated_at": payload.get("updated_at"), "items": []}
    ...
    return {"updated_at": payload.get("updated_at"), "items": merged}
        ...
        records.append(
            {
                ...
                "prev_close": None if pd.isna(row["prev_close"]) else round(float(row["prev_close"]), 2),
                "volume": int(float(row["volume"])) if pd.notna(row.get("volume")) else None,
...
def get_remote_ohlc_history(symbol: str, limit: int = 60) -> list[dict]:
    frame = fetch_remote_stock_daily_history_frame(symbol, limit=limit)
    return format_history_frame(frame, limit=limit)


def get_shanghai_index_history(limit: int = 60) -> list[dict]:
    frame = fetch_index_daily_history_frame("sh000001", limit=limit)
    return format_history_frame(frame, limit=limit)
...
def get_intraday_frame(symbol: str, trade_date: str, period: str = "1") -> pd.DataFrame:
    ...
    frame = frame.loc[frame["day"].dt.strftime("%Y-%m-%d") == trade_date].sort_values("day")
    return frame
...
def build_estimated_intraday_points(day_item: dict, previous_close: float) -> list[dict]:
    ...
    anchors = [
        ("09:30", open_price),
        ("10:30", low_price if rising else high_price),
        ("13:30", high_price if rising else low_price),
        ("15:00", close_price),
    ]
    ...
    return [
        ...
        for time_label, price in anchors
    ]

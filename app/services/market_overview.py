...
US_INDUSTRY_TRANSLATIONS: dict[str, str] = {
    ...
    "Apparel/Footwear Retail": "服装鞋类零售",
    "Electronic Components": "电子元件",
    "Regional Banks": "区域银行",
    "Oil Refining/Marketing": "炼油与营销",
    "Data Processing Services": "数据处理服务",
    "Computer Communications": "计算机通信",
    "Agricultural Commodities/Milling": "农产品与碾磨",
    "Investment Managers": "投资管理",
    "Biotechnology": "生物科技",
    "Medical Specialties": "专科医疗",
    "Electrical Products": "电气设备",
    "Home Improvement Chains": "家装连锁",
    "Computer Peripherals": "计算机外设",
    "Drugstore Chains": "连锁药店",
    "Miscellaneous Commercial Services": "商业服务",
    "Consumer Electronics": "消费电子",
    "Electronic Equipment/Instruments": "电子设备与仪器",
    "Specialty Stores": "专业零售",
    "Trucks/Construction/Farm Machinery": "卡车与工程农机",
    "Medical Distributors": "医疗分销",
    "Recreational Products": "休闲产品",
    "Investment Banks/Brokers": "投行券商",
    "Apparel/Footwear": "服装鞋类",
    "Managed Health Care": "管理式医疗",
    "Restaurants": "餐饮",
    "Real Estate Investment Trusts": "房地产信托",
    "Medical/Nursing Services": "医疗护理服务",
    "Wholesale Distributors": "批发分销",
    "Oil & Gas Production": "油气生产",
    "Specialty Telecommunications": "专业通信服务",
    "Communications": "通信",
    ...
    "Railroads": "铁路",
    "Insurance Brokers/Services": "保险经纪服务",
    "Electronic Technology": "电子科技",
    "Technology Services": "科技服务",
...
def _translate_us_industry_label(value: str | None) -> str:
    ...
    if not normalized:
        return ""
    return US_INDUSTRY_TRANSLATIONS.get(normalized, normalized)


def _parse_signed_percent(value) -> float | None:
    if value in (None, ""):
        return None
...
def _fetch_ths_us_rank_rows(max_pages: int = 6, max_rows: int = 240) -> dict[str, Any]:
    def _builder() -> dict[str, Any]:
        ...
        merged = merged.dropna(subset=["代码", "涨跌幅(%)"])
        merged = merged.loc[merged["代码"] != ""].copy()
...
def _fetch_ths_us_company_profile(symbol: str) -> dict[str, Any]:
    ...
    url = THS_US_COMPANY_TEMPLATE.format(symbol=symbol)
    response = requests.get(url, timeout=20, headers=REQUEST_HEADERS)
    response.raise_for_status()
...
def get_shanghai_intraday_overview() -> dict:
    ...
    try:
        ...
        if not daily_frame.empty:
            ...
            if latest_daily_date == latest_trade_date and len(daily_frame) >= 2:
                previous_close = _safe_float(daily_frame.iloc[-2]["close"])
            else:
                previous_close = _safe_float(daily_frame.iloc[-1]["close"])
    except Exception:
        previous_close = None
    ...
    if baseline_close not in (None, 0) and current_price is not None:
        change = round(current_price - baseline_close, 2)
        change_pct = round((current_price / baseline_close - 1) * 100, 2)

    items = [
        {
            "time": pd.Timestamp(row["date"]).strftime("%H:%M"),
            "price": round(float(row["close"]), 3) if pd.notna(row["close"]) else None,
            "change_pct": round((float(row["close"]) / baseline_close - 1) * 100, 2)
            if pd.notna(row["close"]) and baseline_close not in (None, 0)
            else None,
        }
    ...
    def _builder() -> list[dict]:
        ...
        for item in rank_items:
            ...
            group = grouped.setdefault(
                industry_name,
                {
                    ...
                    "change_pct": None,
                    "sector": "同花顺美股行业聚合",
                    "sector_en": "THS US industry aggregate",
                    "stocks": 0,
                    "source": "同花顺美股涨幅榜 + 公司概况页行业归属",
                    "leader_symbol": None,
                    "leader_name": None,
        ...
        leaders.sort(
            key=lambda item: (
                ...
                item.get("change_pct") if item.get("change_pct") is not None else float("-inf"),
                item.get("stocks") if item.get("stocks") is not None else 0,
            ),
            reverse=True,
        )
        return leaders
...
def get_market_overview(limit: int = 6) -> dict:
    ...
    try:
        us_industry_leaders = get_us_industry_leaders(
            limit=max(limit, 6),
            trade_date=nasdaq_previous.get("trade_date"),
        )
    except Exception as exc:
    ...
    return {
        ...
        "warnings": warnings,
        "shanghai": shanghai,
        "nasdaq_previous": nasdaq_previous,
        "top_sectors": [],
        "us_sector_leaders": us_industry_leaders,
        "us_industry_leaders": us_industry_leaders,
